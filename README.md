...
当前后端提供 ThinkPHP 6 项目所需的 Controller / Logic / Model 分层骨架。业务数据统一读写 MySQL，登录用户、分类、记账记录、月预算都依赖数据库配置；数据库不可用时接口会明确失败，不会静默写入本地文件。
...
5. 本地调试也使用真实微信登录，需要配置同一个小程序的 AppID 和 Secret。
